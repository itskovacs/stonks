...
from models.models import Envelope, Transaction, WatchlistItem
...
from services.portfolio_engine import compute_all_positions, compute_envelope_overview
...
async def get_dashboard(
    ...
    txs_out  = [_tx_out(tx) for tx in txs_orm]

    # ── 2. Compute WAC positions (single pass across all envelopes) ───────────
    ...
    snapshots = await asyncio.gather(
        *[run_in_threadpool(_fetch_ticker_snapshot, t) for t in unique_tickers],
    ...
    total_pnl         = round(total_equity - total_cost_basis, 2)
    total_change_1d   = round(sum(p["change_1d"] for p in enriched_positions), 2)
    yesterday_equity  = total_equity - total_change_1d
    ...
    return {
        ...
        "totals": {
            ...
            "total_pnl":           total_pnl,
            "total_pnl_pct":       round(total_pnl / total_cost_basis * 100, 2) if total_cost_basis else 0.0,
            "total_change_1d":     total_change_1d,
            "total_change_1d_pct": round(total_change_1d / yesterday_equity * 100, 2) if yesterday_equity else 0.0,
...
async def get_trending_tickers(current_user: Annotated[str, Depends(get_current_username)]) -> list[dict]:
    ...
    snapshots = await asyncio.gather(
        *[run_in_threadpool(_fetch_ticker_snapshot, t) for t in trending],
        return_exceptions=True,
    )
